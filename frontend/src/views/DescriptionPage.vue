...
<!--      <my-button text="Chat"  @click="this.showAgent=true" style="width: 150px; height: 40px; margin-right: 20px"></my-button>-->
    ...
    <div v-show='writingFinished' style="display: grid; grid-template-columns: 1fr 2fr; grid-column-gap: 30px;margin-top: 20px">
        <div class="panel" style="width: 100%; box-sizing: border-box; align-items: flex-start; flex-grow: 0">
          <p class="primaryText">Narrator's Fear:</p>
        </div>
        <div class="panel" style="width: 100%; box-sizing: border-box; align-items: flex-start; flex-grow: 0">
          <p class="primaryText">{{narratorFearResult}}</p>
        </div>
  
      </div>
    
      <!-- defining monents sections -->
      <div v-show='writingFinished' style="display: grid; grid-template-columns: 1fr 2fr; grid-column-gap: 30px;margin-top: 20px">
        <div class="panel" style="width: 100%; box-sizing: border-box; align-items: flex-start; flex-grow: 0">
          <p class="primaryText">Defining Moment:</p>
        </div>
        <div class="panel" style="width: 100%; box-sizing: border-box; align-items: flex-start; flex-grow: 0">
          <p class="primaryText">{{defineMomentResult}}</p>
        </div>
      ...
      <div v-show='writingFinished' style="display: grid; grid-template-columns: 1fr 2fr; grid-column-gap: 30px;margin-top: 20px">
        <div class="panel" style="width: 100%; box-sizing: border-box; align-items: flex-start; flex-grow: 0">
          <p class="primaryText">Image Presentation:</p>
        </div>
        <div class="panel" style="width: 100%; box-sizing: border-box; align-items: flex-start; flex-grow: 0">
          <p class="primaryText">{{imagePresentationResult}}</p>
        </div>
...
export default {
  ...
  methods:{
    finish() {
      ...
      localStorage.setItem('location',this.locationResult)
      localStorage.setItem('character',this.characterResult)
      localStorage.setItem('imagePresentation',this.imagePresentationResult)
    ...
    async analysis(){
      ...
      const fearResponse = await createCompletion({
        ...
        prompt: "What's the narrator’s fear in this nightmare? The dream is: " + this.description,
        max_tokens: 1000,
        temperature: 0,
        top_p: 1,
        n: 1,
      });
      this.narratorFearResult = fearResponse.data.choices[0].text.trim()

      const emotionResponse = await createCompletion({
        model: "text-davinci-003",
        prompt: "What's the narrator emotions in the dream? Using 5 words to describe. Separate words with comma and no not end with period. The dream is: " + this.description,
        max_tokens: 1000,
        temperature: 0,
        top_p: 1,
        n: 1,
      ...
      const locationResponse = await createCompletion({
        model: "text-davinci-003",
        prompt: "What's the locations in the dream? Using 5 words to describe. Separate words with comma and no not end with period. The dream is: " + this.description,
        max_tokens: 1000,
        temperature: 0,
        top_p: 1,
        n: 1,
      });
      this.locationResult = locationResponse.data.choices[0].text.trim()

      const characterResponse = await createCompletion({
        model: "text-davinci-003",
        prompt: "What's the characters in the dream? Using 5 words to describe. Separate words with comma and no not end with period. The dream is: " + this.description,
        max_tokens: 1000,
        temperature: 0,
        top_p: 1,
        n: 1,
      });
      this.characterResult = characterResponse.data.choices[0].text.trim()

      const deinineMomentResponse = await createCompletion({
        model: "text-davinci-003",
        prompt: "What's the defining moment in this nightmare? The dream is: " + this.description,
        max_tokens: 100,
        temperature: 0,
        top_p: 1,
        n: 1,
      });
      this.defineMomentResult = deinineMomentResponse.data.choices[0].text.trim()

      const imageResponseResponse = await createCompletion({
        model: "text-davinci-003",
        prompt: "How to use an image to present this defining moment? Please provide the text prompt for image generation " + this.defineMomentResult,
        max_tokens: 100,
        temperature: 0,
        top_p: 1,
        n: 1,
      });
      this.imagePresentationResult = imageResponseResponse.data.choices[0].text.trim()
